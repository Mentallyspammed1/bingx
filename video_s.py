    ...
    try:
        ...
        if i_el:
            attrs_to_try = [
                ...
                "data-src", "data-original", "data-thumb", "data-poster", "data-lazy-src", "src",
            ]
...
async def build_gallery(
    ...
    for res in results:
        ...
        views_text = "" if views == "N/A" else html.escape(views)
...
def main() -> None:
    ...
    driver = None
    needs_js = bool(cfg.get("requires_js"))                           want_selenium = args.use_selenium or needs_js                 
    if want_selenium:
        if not SELENIUM_AVAILABLE:
            if needs_js:
