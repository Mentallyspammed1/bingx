...
async function testGifScraper() {
    log.info('Initializing MotherlessScraper for GIF (Image) test...');
    const options = {
        query: 'animation',
    ...
    log.info('Testing MotherlessScraper - searchGifs("animation", 1)...');
    try {
        const results = await scraper.searchGifs('animation', 1);
        ...
        if (results && results.length > 0) {
            ...
            results.slice(0, 3).forEach((item, index) => {
                ...
                console.log(`Preview Video (image file): ${item.preview_video}`); // Expecting this to be an image URL
            ...
            log.info('No Image/GIF results found by the scraper.');
        } else {
