...
# --- Cost Estimation Sigils (Approximation for Gemini Models) ---
...
COST_DATA = {
    ...
    "default": {"input": 3.50, "output": 10.50}  # Fallback for unknown models
}
CHARS_PER_TOKEN = 4

# --- Global State & Logging Setup ---
...
class ScriptContext:
    ...
    has_changes: bool = False  # Flag for --diff-only mode

# This global context will be initialized once in main()
# It helps avoid passing the same object to every single function call.
# While not pure, it's a pragmatic solution for a script of this nature.
ctx: ScriptContext | None = None

class NeonFormatter(logging.Formatter):
    """A logging formatter with a vibrant neon theme."""
    FORMATS = {
        logging.DEBUG: f"{Fore.LIGHTMAGENTA_EX}[DBG]{Style.RESET_ALL} %(message)s",
        logging.INFO: f"{Fore.LIGHTCYAN_EX}[INFO]{Style.RESET_ALL} {Fore.LIGHTBLUE_EX}%(message)s{Style.RESET_ALL}",
        logging.WARNING: f"{Fore.LIGHTYELLOW_EX}{Style.BRIGHT}[WARN]{Style.RESET_ALL} {Fore.LIGHTYELLOW_EX}%(message)s{Style.RESET_ALL}",
        logging.ERROR: f"{Fore.LIGHTRED_EX}{Style.BRIGHT}[ERROR]{Style.RESET_ALL} {Fore.LIGHTRED_EX}%(message)s{Style.RESET_ALL}",
        logging.CRITICAL: f"{Fore.LIGHTWHITE_EX}{Style.BRIGHT}[CRIT]{Style.RESET_ALL} {Fore.LIGHTRED_EX}%(message)s{Style.RESET_ALL}",
    }
...
def log_success(message: str): logger.info(f"{Fore.LIGHTGREEN_EX}{Style.BRIGHT}{message}{Style.RESET_ALL}")
...
def get_api_key():
    ...
    try:
        api_key_input = input(f"{Fore.LIGHTBLUE_EX}{Style.BRIGHT}Enter your Gemini API key: {Style.RESET_ALL}").strip()
...
def check_dependencies():
    ...
    deps = {"ruff": "Python linter & formatter", "shellcheck": "Shell script linter"}
    missing = []
...
def create_temp_dir() -> bool:
    ...
    try:
        ...
        if base_tmp_dir and not Path(base_tmp_dir).is_dir():
             log_warning(f"Custom temp dir {base_tmp_dir} does not exist, using system default.")
             base_tmp_dir = None

        ctx.temp_dir = Path(tempfile.mkdtemp(prefix="gemini_review_", dir=base_tmp_dir))
...
def cleanup_temp_dir():
    ...
    if ctx and ctx.temp_dir and ctx.temp_dir.exists():
        try:
            ...
            log_warning(f"Could not banish temporary sanctum '{ctx.temp_dir}': {e}")
...
def backup_file(file_path: Path) -> bool:
    ...
    backup_path = file_path.with_name(f"{file_path.name}.{timestamp}{BACKUP_SUFFIX}")
    try:
...
def get_file_type_from_extension(file_path: Path) -> str | None:
    ...
    ext_map = {
        ...
        ".php": "php", ".swift": "swift", ".kt": "kotlin", ".cs": "csharp", ".vue": "vue",
        ".ipynb": "json"
    }
...
def _split_python(content: str) -> list[str]:
    ...
    try:
        ...
        if not tree.body: # Handle files with only comments/whitespace
            return [content] if content.strip() else []
        chunks_raw = [astor.to_source(node) for node in tree.body]
        return [c for c in chunks_raw if c.strip()]
    except (SyntaxError, ValueError) as e:
        log_warning(f"AST parsing failed: {e}. Falling back to paragraph split.")
...
def _split_shell(content: str) -> list[str]:
    ...
    # Split by lines that start function/control blocks, or by empty lines
    chunks_raw = re.split(r'(?m)(^\s*(?:function\s|if\s|for\s|while\s|case\s|until\s|select\s) | ^\s*$)', content)
    recombined = []
    ...
    return [c for c in recombined if c.strip()] or ([content] if content.strip() else [])
...
def split_file_into_chunks(input_path: Path, lang_hint: str | None) -> list[Path]:
    ...
    for chunk_content in chunks_raw:
        if max_tokens and (len(chunk_content) // CHARS_PER_TOKEN) > max_tokens:
            ...
            for line in lines:
                ...
                if sub_chunk_tokens + line_tokens > max_tokens and sub_chunk_lines:
...
def run_pre_check(content: str, lang_hint: str) -> bool:
    ...
    if lang_hint == "python" and shutil.which("ruff"):
        ...
        if proc.returncode != 0:
            log_warning(f"Ruff check found issues:\n{proc.stdout.decode('utf-8')}")
            return False
        ...
        proc = subprocess.run(cmd, input=content.encode("utf-8"), capture_output=True, check=False)
        if proc.returncode != 0:
            log_warning(f"ShellCheck found issues:\n{proc.stdout.decode('utf-8')}")
            return False
...
def process_chunk_with_api(original_chunk_path: Path, output_chunk_path: Path, file_info: str) -> tuple[str, str, Path, int, int] | None:
    ...
    current_lang_hint = ctx.args.lang or get_file_type_from_extension(Path(file_info))
    ...
    cache_key_str = prompt_text + original_content + ctx.args.model + str(ctx.args.temperature)
    ...
    if not ctx.args.force and cache_file.exists():
        ...
        return final_content, original_content, original_chunk_path, 0, output_tokens

    json_payload = {
        "contents": [{"parts": [{"text": prompt_text}]}],
        "generationConfig": {"temperature": ctx.args.temperature, "maxOutputTokens": 8192}
    }
    input_tokens = len(prompt_text) // CHARS_PER_TOKEN
    api_url = f"{API_BASE_URL}/{ctx.args.model}:generateContent?key={ctx.api_key}"
    ...
    for attempt in range(1, ctx.args.retries + 1):
        try:
            response = requests.post(
                ...
                json=json_payload,
                timeout=(ctx.args.connect_timeout, ctx.args.read_timeout)
            )
            ...
            if ctx.args.pre_check and final_content.strip() != original_content.strip():
                log_info(f"Running post-API check for '{chunk_name}'...")
                if not run_pre_check(final_content, current_lang_hint):
                    log_warning(f"Post-API check failed for '{chunk_name}'. Reverting to original.")
            ...
            log_warning(f"Timeout for '{chunk_name}'. Retrying in {ctx.args.retry_delay}s...")
            time.sleep(ctx.args.retry_delay)
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 429:
                log_warning(f"Rate limit reached. Waiting {API_RATE_LIMIT_WAIT}s...")
                time.sleep(API_RATE_LIMIT_WAIT)
            elif e.response.status_code >= 500:
                log_warning(f"API server error ({e.response.status_code}). Retrying...")
                time.sleep(ctx.args.retry_delay)
            else:
                log_error(f"API Error {e.response.status_code} for '{chunk_name}'. Not retryable. Details: {e.response.text[:200]}")
                return None
    ...
    log_error(f"Failed to enhance fragment '{chunk_name}' after {ctx.args.retries} attempts. Using original.")
    output_chunk_path.write_text(original_content, encoding="utf-8")
...
def display_diff(original_content: str, new_content: str, file_path_for_display: str):
    ...
    for line in diff_lines:
        line = line.rstrip()
        if line.startswith("+++") or line.startswith("---"): print(f"{Fore.LIGHTBLUE_EX}{Style.BRIGHT}{line}{Style.RESET_ALL}")
        elif line.startswith("+"): print(f"{Fore.LIGHTGREEN_EX}{line}{Style.RESET_ALL}")
        elif line.startswith("-"): print(f"{Fore.LIGHTRED_EX}{line}{Style.RESET_ALL}")
        elif line.startswith("@@"): print(f"{Fore.LIGHTMAGENTA_EX}{line}{Style.RESET_ALL}")
        else: print(line)

def reassemble_output(all_original_chunks: list[Path], file_path: Path, to_stdout: bool) -> str:
    ...
    for original_chunk_file in all_original_chunks:
        ...
        try:
            ...
            log_error(f"Error reading fragments for {chunk_basename}: {e}. Using original.")
            final_content_parts.append(original_content)
            continue

        if ctx.args.interactive and not accept_all and original_content.strip() != new_content.strip() and not ctx.args.dry_run:
            ...
            while True:
                try:
                    resp = input(f"{Fore.LIGHTGREEN_EX}Accept? {Style.BRIGHT}(y/n/e[dit]/a[ccept all]): {Style.RESET_ALL}").lower().strip()
                except (EOFError, KeyboardInterrupt):
                    print()
                    resp = "n"
...
def display_final_summary():
    ...
    input_cost = (ctx.total_input_tokens / 1_000_000) * model_costs["input"]
    output_cost = (ctx.total_output_tokens / 1_000_000) * model_costs["output"]
...
def get_files_from_input_dir() -> list[tuple[Path, Path]]:
    ...
    default_skip_ext = {".png", ".jpg", ".jpeg", ".gif", ".svg", ".zip", ".tar", ".gz",
                        ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".sqlite3", ".db", ".lock",
                        ".log", ".tmp", ".bak", ".pyc", ".o", ".a", ".so", ".dll"}
    all_skip_ext = set(ext.lower() for ext in ctx.args.skip_ext) | default_skip_ext
    ...
    for root_str, _, files in os.walk(input_dir):
        ...
        for file_name in files:
            ...
            if file_path.suffix.lower() in [BACKUP_SUFFIX, CHECKPOINT_SUFFIX] or file_path.suffix.lower() in all_skip_ext:
...
def process_single_file_workflow(original_file_path: Path, output_file_path: Path):
    ...
    file_lang_hint = ctx.args.lang or get_file_type_from_extension(original_file_path)
    if not file_lang_hint:
        log_warning(f"Could not infer language for '{original_file_path.name}'.")

    all_chunks_raw_paths = split_file_into_chunks(original_file_path, file_lang_hint)
    if not all_chunks_raw_paths:
        if original_content.strip(): # If file has content but no chunks were made
             log_error(f"Division of text for '{original_file_path.name}' failed. Skipping file.")
        else: # Handle empty file case
             final_content = ""
             if not ctx.args.dry_run:
                 if is_stdout: sys.stdout.write(final_content)
                 else: output_file_path.write_text(final_content, encoding="utf-8")
        return

    log_info(f"Total fragments for '{original_file_path.name}': {len(all_chunks_raw_paths)}.")
    ...
    final_content = reassemble_output(all_chunks_raw_paths, original_file_path, is_stdout)
    ...
    if ctx.args.dry_run:
        ...
        try:
            ...
            log_success(f"Tome woven successfully. Saved to '{output_file_path}'.")
        except OSError as e:
            log_error(f"Failed to write final tome to '{output_file_path}': {e}")
...
def list_available_models():
    ...
    if not ctx.api_key:
        ...
        return
    try:
        response = requests.get(f"https://generativelanguage.googleapis.com/v1beta/models?key={ctx.api_key}")
        response.raise_for_status()
        models = response.json().get("models", [])
        print(f"{Fore.LIGHTCYAN_EX}{Style.BRIGHT}--- Available Gemini Models ---{Style.RESET_ALL}")
        for model in sorted(models, key=lambda m: m['name']):
             model_name = model['name'].replace('models/', '')
             cost_info = COST_DATA.get(model_name)
             cost_str = f" (Input: ${cost_info['input']}/M, Output: ${cost_info['output']}/M)" if cost_info else ""
             print(f"  - {Fore.LIGHTGREEN_EX}{model_name}{Style.RESET_ALL}{cost_str}")
...
"""
    ...
    # Use a flag to check if any processing arg is provided
    ...
    output_group.add_argument("-o", "--output-file", type=Path, help="Output file path. Default: stdout for single file.")
    output_group.add_argument("--output-dir", type=Path, help="Output directory. Required if not using --in-place.")
    output_group.add_argument("--in-place", action="store_true", help="Modify files in-place (creates backups).")

    parser.add_argument("--stdout", action="store_true", help="Force all output to stdout, overriding file-based output.")
    ...
    parser.add_argument("-j", "--jobs", type=int, default=DEFAULT_MAX_JOBS, help=f"Max parallel API requests (default: {DEFAULT_MAX_JOBS}).")
    parser.add_argument("-f", "--force", action="store_true", help="Force overwrite of existing output files.")
    parser.add_argument("--raw", action="store_true", help="Output raw API response, bypassing code extraction.")
    parser.add_argument("--log-level", choices=log_level_map.keys(), default="INFO", help="Set logging level.")
    parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose logging (alias for --log-level DEBUG).")
    parser.add_argument("--interactive", action="store_true", help="Interactively review changes for each fragment.")
    parser.add_argument("--pre-check", action="store_true", help="Run pre-API checks (e.g., ruff, shellcheck). Skips API call if original code is valid.")
    parser.add_argument("--max-chunk-tokens", type=int, help="Max estimated tokens per chunk for splitting.")
    parser.add_argument("--connect-timeout", type=int, default=DEFAULT_CONNECT_TIMEOUT, help=f"Connection timeout (default: {DEFAULT_CONNECT_TIMEOUT}s).")
    parser.add_argument("--read-timeout", type=int, default=DEFAULT_READ_TIMEOUT, help=f"Read timeout (default: {DEFAULT_READ_TIMEOUT}s).")
    parser.add_argument("--retries", type=int, default=MAX_RETRIES, help=f"API retry attempts (default: {MAX_RETRIES}).")
    parser.add_argument("--retry-delay", type=int, default=RETRY_DELAY_SECONDS, help=f"Delay between retries (default: {RETRY_DELAY_SECONDS}s).")
    parser.add_argument("--skip-ext", nargs="*", default=[], help="File extensions to skip (e.g., .log .txt).")
    parser.add_argument("--dry-run", action="store_true", help="Show changes but do not write files. Use with --diff-only to get a patch-like output.")
    parser.add_argument("--diff-only", action="store_true", help="Show a diff of changes and exit with status 0 (no changes) or 1 (changes). Implies --dry-run.")
    ...
    if args.prompt_file:
        ...
        # Set default prompt if neither custom option is used
        args.custom_prompt_template = (
            "You are an expert software engineer. Review and provide syntax corrections for the following code chunk from '{file_info}'.\n"
